...
public class Level2 extends LevelState {
	...
	int MAX_JUMP = 30;			// Time the player can jump before falling.
	...
	int PLATFORM_SPACE_X = 0;
	int PLATFORM_SPACE_Y = 150;
	int PLATFORM_RANDOM_X = 200;
	int PLATFORM_RANDOM_Y = 25;
	...
	public void init() {
		entities.add(new EntitySloppy(100, 10, 50, 50, "Player"));
		entities.add(new EntitySloppy(90, 190, 70, 10, "Plataforma"));
...
//		setBackgroundEnabled(false);
	...
	public void update() {
		if (gameOver == true){
			...
			if (entities.get(0).cornerIsColliding.get(Dir4DEnum.BOTTOM_LEFT) ||
				...
				int newPlatX = rand.nextInt(PLATFORM_RANDOM_X) + PLATFORM_SPACE_X;
				int newPlatY = rand.nextInt(PLATFORM_RANDOM_Y) + PLATFORM_SPACE_Y;
				entities.get(1).body.x = newPlatX;
				entities.get(1).body.y = newPlatY;
			}
	...
	public void draw(Graphics2D g) {
		...
		if (gameOver) {
			...
			g.drawString(String.format("Game Over! Score: %d", levelScore), GAME_OVER_X, GAME_OVER_Y);
		}
